...
A collection of triangles without any particular mathematical structure is called \textit{triangle mesh} (see example in Fig. \ref{fig:mesh-3d-armadillo}). To derive a global parameterization for an entire triangle mesh we can define a 2D position for each vertex. Let $\mathcal{M}$ be a triangle mesh that consists of a geometric and topological component represented by a graph structure with a set of vertices $\mathcal{V} = \{ v_1, ..., v_V \}$ and a set of triangular faces connecting them $\mathcal{F} = \{ f_1, ... , f_F \}$ with $f_i \in \mathcal{V} \times \mathcal{V} \times \mathcal{V}$. The connectivity of a triangle mesh can be expressed in terms of the edges of the respective graph $\mathcal{E} = \{ e_1, ..., e_E \}$ where $e_i \in \mathcal{V} \times \mathcal{V}$.
...
\begin{figure}[h!]
  ...
  \endminipage
\caption{On the left: a 3D triangle mesh filled by face shading. On the right: structure of 3D triangle meshes where you can see the connectivity of a triangle mesh.}\label{fig:mesh-3d-armadillo}
\end{figure}
...
\caption{Phong lighting model. $p$ is the point illuminated by the ray ($l-p$). $v$ represents the view position and $l$ the lighting source. \cite{SLIDE:ICORSI}} \label{figure:lighting}
\end{figure}
...
Let $r$ be our reflected light vector defined as $r = 2 n \; \cdot <n, l - p> - (l-p)$ and $\alpha$ the angle between that vector and the view direction $(v - p)$ as depicted in Figure \ref{figure:lighting}.
...
Linear interpolation is a method that returns equal spacing between the interpolated values. Given two numbers $n_1$ and $n_2$ (the start and final values of the interpolant), a linear interpolation can be carried out using a parameter $t$ ($t \in [0,1]$). \cite{WEBSITE:interpolation}

$$ n = n_1 + t (n_2 - n_1)$$


The standard linearly interpolated visualization is carried out by passing three attributes (colors) to each vertex of a triangle. OpenGL will interpolate these colors linearly by exploiting the barycentric coordinates that tell us how much of each color is being mixed at any position.
Given a triangle $[p_1, p_2, p_3]$, where the color blue is passed to vertex $p_1$, red to $p_2$, and green to $p_3$, let $w_1$ be the blue area, $w_2$ the red area and $w_3$ the green area (see Fig. \ref{fig:barycentric-coord}). Let us compute the value at $p$ using \textit{barycentric interpolation} $$(w_1p_1 + w_2p_2 + w_3p_3)/W$$ where $W$ is the area of the triangle $[p_1, p_2, p_3]$.
...
Given a triangle $[p_1, p_2, p_3]$, the lighting is computed using the normal $n$: $$\widehat{n} = (p_2 - p_1) \times (p_3 - p_1) \;\;\;\;\;\;\;\; n = \frac{ \widehat{n} } { ||\widehat{n}|| } $$ at $p= (p_1 + p_2 + p_3)/3$. This color is then used for all pixels of the triangle. Flat shading gives objects with flat facets.
\cite{SLIDE:ICORSI}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
These colors are then linearly interpolated $c = {\mu}_1 c_1 + {\mu}_2 c_2 + {\mu}_3 c_3$. Gouraud shading gives objects that appear more smooth. \cite{SLIDE:ICORSI}
