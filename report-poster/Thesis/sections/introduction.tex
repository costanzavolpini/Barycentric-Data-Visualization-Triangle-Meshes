...
The standard linear interpolated visualisation is made passing three attributes (colors) to each vertex of a triangle. OpenGL will interpolate these colors linearly thanks to the barycentric coordinates that will tell how much of each color is being mixed at any position.
...
\textit{Flat shading} is a way to compute the color at each pixel (at a corner or the barycentre) using the triangle normal.
...
The lighting is computed at $p_i$ using normal $n_i$ this applied to each corner return the color values $c_1, c_2, c_3$.
